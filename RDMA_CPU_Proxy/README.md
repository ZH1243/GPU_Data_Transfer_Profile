...
For each remote peer slot, the expert routing table contains eight packed mask arrays indexed by destination GPU. The source GPU's array and any GPU indices outside `num_gpus_per_node` remain empty. Each destination array follows the sorted GPU routing table's token order but omits tokens whose routing bit for that GPU is zero. Every retained mask occupies exactly `num_of_experts_per_GPU / 8` bytes. Each expert bit is sampled independently with `expert_routing_probability`, and expert 0 is bit 0.

Alongside the packed arrays, a prefix-rank table records the compressed array index at every original token boundary. A batch `[batch_start_token, batch_end_token)` therefore maps to one contiguous expert-mask range `[start_index, end_index)` for each destination, including dynamic and out-of-order batches. The forwarding thread hands only this range to the notification dispatch thread. The dispatch thread copies the complete packed range into the shared-memory notification with one `memcpy`; the forwarding thread does not build or copy an expert-mask vector.
...
In expert-routing notification mode, each receiver proxy also allocates two private scratch buffers in its GPU's global memory during initialization: one 16 KiB buffer and one 8-byte buffer. Reusable CUDA-pinned host buffers of the same sizes are filled with byte value `1`. For every dequeued expert notification, the receiver enqueues a 16 KiB host-to-device `cudaMemcpyAsync` followed by an 8-byte host-to-device `cudaMemcpyAsync` on one dedicated nonblocking stream. Stream ordering preserves that sequence, and the notification queue tail advances after both operations have been enqueued. Shutdown synchronizes the stream before freeing either the host or GPU buffers. Mock mode performs the same ordered operations with host `memcpy`.

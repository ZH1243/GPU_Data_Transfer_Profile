...
In expert-routing notification mode, each receiver proxy also allocates two private scratch buffers in its GPU's global memory during initialization: one 1 MiB buffer and one 8-byte buffer. Reusable host buffers of the same sizes are filled with byte value `1`. For every dequeued expert notification, the receiver performs a synchronous 1 MiB host-to-device `cudaMemcpy` followed by a synchronous 8-byte host-to-device `cudaMemcpy`. The notification queue tail advances only after both copies finish. Mock mode performs the same ordered operations with host `memcpy`.

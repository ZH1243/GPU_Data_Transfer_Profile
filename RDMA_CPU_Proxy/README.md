...
In expert-routing notification mode, each receiver proxy also allocates two private scratch buffers in its GPU's global memory during initialization: one 1 MiB buffer and one 8-byte buffer. Reusable CUDA-pinned host buffers of the same sizes are filled with byte value `1`. For every dequeued expert notification, the receiver enqueues a 1 MiB host-to-device `cudaMemcpyAsync` followed by an 8-byte host-to-device `cudaMemcpyAsync` on one dedicated nonblocking stream. Stream ordering preserves that sequence, and the notification queue tail advances after both operations have been enqueued. Shutdown synchronizes the stream before freeing either the host or GPU buffers. Mock mode performs the same ordered operations with host `memcpy`.

#!/usr/bin/env python3
"""Analyze source-side NVLink peer-copy activity from an Nsight Systems report.

The script accepts either an .nsys-rep file or an exported .sqlite file. When an
.nsys-rep input is used, a sibling .sqlite file is reused if present; otherwise
the script tries to run `nsys export`.
"""
...
DEFAULT_REPORT = Path(__file__).with_name("1*1m.nsys-rep")
DEFAULT_GPU_BUS_ID = "0000:03:00.0"
DEFAULT_SKIP_WARMUP_COPIES = 10
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "NVLink RX/TX metrics for one collected GPU."
        )
    )
    parser.add_argument(
        "report",
        ...
        help="Input .nsys-rep or .sqlite file. Defaults to ./1*1m.nsys-rep.",
    )
    parser.add_argument(
        "--gpu-bus-id",
    ...
    parser.add_argument(
        "--skip-warmup-copies",
        type=int,
        default=DEFAULT_SKIP_WARMUP_COPIES,
        help=(
            "Ignore this many earliest source-side Memcpy PtoP events before "
            f"computing all statistics. Default: {DEFAULT_SKIP_WARMUP_COPIES}."
        ),
    )
    parser.add_argument(
        "--json",
...
def build_summary(
    ...
    bus_id: str,
    peak_gb_s: float | None,
    skip_warmup_copies: int,
) -> dict[str, Any]:
    if skip_warmup_copies < 0:
        raise ValueError("--skip-warmup-copies must be non-negative")
    ...
    events = all_events[skip_warmup_copies:]
    ...
    paired_api_count = sum(1 for event in events if event.api_end_ns is not None)
    ...
    return {
        ...
        "counts": {
            ...
            "skipped_warmup_source_memcpy_ptop_count": min(
                skip_warmup_copies, len(all_events)
            ),
            "source_memcpy_ptop_count": len(events),
            "paired_cuda_memcpy_peer_async_count": paired_api_count,
            "unpaired_source_memcpy_ptop_count": len(events) - paired_api_count,
        },
        "throughput": {
...
def print_text(summary: dict[str, Any]) -> None:
    ...
    print("1. Counts")
    print(
        "   cudaMemcpyPeerAsync / Memcpy PtoP (source) pairs: "
        f"{counts['paired_cuda_memcpy_peer_async_count']}"
    )
    ...
    print(
        "   "
        f"{fmt_float(gap['average_us'])} us "
        f"over {gap['count']} gaps"
    )
    print()
    ...
    print()
    print("5. Average waiting time: Memcpy PtoP start - cudaMemcpyPeerAsync end")
    print(
        "   "
        f"{fmt_float(wait['average_us'])} us "
        f"over {wait['count']} paired events"
    )
    print()
